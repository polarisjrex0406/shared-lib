...
type ReferralEarningRepository interface {
	...
	Create(tx *gorm.DB, referralEarning *entities.ReferralEarning) error
...
func (r *referralEarningRepository) BeginTx() *gorm.DB {
	return r.DB.Begin()
}

func (r *referralEarningRepository) Create(tx *gorm.DB, referralEarning *entities.ReferralEarning) error {
	dbInst := r.DB
	if tx != nil {
		dbInst = tx
	}
	result := dbInst.Create(referralEarning)
	return result.Error
}
